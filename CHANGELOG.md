...
+ [调整] 移除页面入口禁用持久缓存的响应头，缓存策略交回 Next.js 与部署平台处理。
+ [修复] 提示词接口响应缺少列表字段时使用空数组兜底，避免首页渲染报错。
+ [修复] Docker 构建阶段改用 Node 执行 Next 构建，避免部分环境中 `bun run build` 以 `exit code 132` 退出。
+ [修复] 画布默认生图张数的生成偏好空值回退保持为 1 张。

## v0.4.0.3

+ [新增] 新增远程图片同源代理兜底，修复无 CORS 响应头图片 URL 无法保存到本地图片存储的问题。
+ [调整] 画布默认生图张数从 3 张调整为 1 张。
